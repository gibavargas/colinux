...
#   sudo ./mkimage.sh --profile colinux-lite-gui --arch x86_64 \
#       --outdir ./out --repository http://dl-cdn.alpinelinux.org/alpine/v3.21/main
# =============================================================================

profile_colinux_lite_gui() {
    profile_base
    ...
    # wrappers, OpenRC services, GUI configs) is never packaged into the ISO.
    ...
    # Trace: mkimg.colinux-lite-gui.sh loaded for $ARCH
}
...
profile_colinux_lite_gui_create_image() {
    local img_size_mb="${COLINUX_IMG_SIZE:-900}"
...
profile_colinux_lite_gui_build_kernel() {
...
profile_colinux_lite_gui_install_bootloader() {
...
profile_colinux_lite_gui_install_extlinux() {
...
profile_colinux_lite_gui_create_image_ext() {
...
# =============================================================================
profile_colinux_lite_gui_overlay() {
    ...
    if ! grep -q '^codex:' "$WORKDIR"/etc/passwd 2>/dev/null; then
        echo "codex:x:1000:1000:CoLinux GUI User:/home/codex:/bin/bash" >> "$WORKDIR"/etc/passwd
...
CONF
    ...
    local overlay_dir="${mkimg_profiles_dir:-.}/colinux-lite-gui/overlay-gui"
